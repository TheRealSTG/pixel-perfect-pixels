  ...
  <g transform={`translate(${x}, ${y}) rotate(${rotation}) scale(${scale})`}>
    {style === "pixel" ? (
      ...
      <>
        ...
        {/* Branching stems with tiny flowers */}
        {[
          { bx: -8, by: -8, stems: [[-12,-14],[-6,-16],[-10,-10]] },
          { bx: 6, by: -4, stems: [[10,-10],[8,-14],[4,-8]] },
          { bx: -3, by: -12, stems: [[-6,-18],[-1,-20],[2,-16]] },
          { bx: 4, by: -10, stems: [[8,-16],[5,-18],[2,-12]] },
        ].map((branch, bi) => (
          <React.Fragment key={bi}>
            <line x1="0" y1={branch.by + 12} x2={branch.bx} y2={branch.by}
              stroke="#8AA878" strokeWidth={0.5} />
            {branch.stems.map(([sx,sy], si) => (
              <React.Fragment key={si}>
                <line x1={branch.bx} y1={branch.by} x2={sx} y2={sy}
                  stroke="#A0B890" strokeWidth={0.3} />
                <circle cx={sx} cy={sy} r={1.2} fill={color} opacity={0.9} />
                <circle cx={sx} cy={sy} r={0.5} fill={accentColor} opacity={0.6} />
              </React.Fragment>
            ))}
          </React.Fragment>
        ))}
...
export const Fern: React.FC<FlowerProps> = ({
  ...
  <g transform={`translate(${x}, ${y}) rotate(${rotation}) scale(${scale})`}>
    {style === "pixel" ? (
      ...
      <>
        {/* Central frond stem */}
        <path d={`M 0 -14 Q 1 8, 0 38`} stroke={accentColor} strokeWidth={1.2} fill="none" strokeLinecap="round" />
        {/* Alternating pinnae (leaflets) */}
        {[-12, -8, -4, 0, 4, 8, 12, 16, 20, 24].map((py, i) => {
          const side = i % 2 === 0 ? -1 : 1;
          const length = 8 - Math.abs(py + 2) * 0.2;
          if (length < 2) return null;
          return (
            <path key={py}
              d={`M 0 ${py} Q ${side * length * 0.6} ${py - 2}, ${side * length} ${py - 1} Q ${side * length * 0.5} ${py + 1}, 0 ${py}`}
