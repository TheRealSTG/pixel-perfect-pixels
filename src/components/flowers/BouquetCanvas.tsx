    ...
    >
      <defs>
        ...
        <filter id="wc-bleed" x="-20%" y="-20%" width="140%" height="140%">
          <feTurbulence type="fractalNoise" baseFrequency="0.7" numOctaves="3" seed="4" result="noise" />
          <feDisplacementMap in="SourceGraphic" in2="noise" scale="3.2" result="disp" />
          <feGaussianBlur in="disp" stdDeviation="0.85" />
        </filter>
        ...
        <filter id="bot-ink" x="-10%" y="-10%" width="120%" height="120%">
          {/* Strong desaturate — vintage print feel */}
          <feColorMatrix type="saturate" values="0.55" result="desat" />
          {/* Warm sepia shift + slight darken */}
          <feColorMatrix in="desat" values="0.85 0.05 0 0 0.02  0 0.82 0 0 0.04  0 0 0.7 0 0.02  0 0 0 1 0" result="muted" />
          {/* Ink outline: dilate then darken */}
          <feMorphology in="muted" operator="dilate" radius="0.7" result="dilated" />
          <feColorMatrix in="dilated" values="0.18 0 0 0 0.04  0 0.18 0 0 0.06  0 0 0.18 0 0.03  0 0 0 1 0" result="ink" />
        ...
        <filter id="pixel-crunch" x="-5%" y="-5%" width="110%" height="110%">
          ...
          {/* Subtle pixel grid jitter so edges read as crunchy blocks */}
          <feTurbulence type="fractalNoise" baseFrequency="0.95" numOctaves="1" seed="3" result="grid" />
          <feDisplacementMap in="q" in2="grid" scale="1.2" />
        </filter>
      ...
      {isPixel && (
        <rect x="-100" y="-100" width="200" height="200" fill="url(#pixel-scan)" pointerEvents="none" />
      )}
    </svg>
  );
};
